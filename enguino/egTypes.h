...
typedef struct {
  ...
  short gfactor;    // 0-4000 vertical gauge, 0-8000 horizontal gauge, 0-2400 round gauge
  short lowWarning;
  short lowCaution;
  short highCaution;
  short highWarning;
...
typedef struct {
  ...
  string *labelValues;
  const short *labelPts;    // prescaled from low,high and unscaled pts
  byte n_labels;

  string *regionColors;
  const short *regionEndPts;  // prescaled from low,high and unscaled pts
  byte n_regions;
...
#define ARCY(x) short(.5-(13000 * SIN2(((x)*4./3.-1./6.) * PI)))
#define VSEG(x) short(.5+4000*(x))
#define HSEG(x) short(.5+8000*(x))
...
#define SCALE(factor)   (short)((factor)*(1<<divisor) + 0.5)  // Converts a floating point factor to integer
#define GRNG(range)     SCALE(1000.0/(range))               // Used for gfactor, given range returns gfactor that will result in 0-1000
#define GMIN(low)       (short)(-((low) + 0.5))
